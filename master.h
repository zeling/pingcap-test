...
public:
    ...
    master(std::string input, size_t n_shards, size_t mem_high_water_mark, size_t top_k)
        : _n_shards(n_shards)
        , _mem_usage(0)
        , _mem_high_water_mark(mem_high_water_mark)
        , _top_k(top_k)
        , _result(top_k)
    ...
    heap_type::iterator result_begin() {
        return _result.begin();
    }

    heap_type::iterator result_end() {
        return _result.end();
    }
